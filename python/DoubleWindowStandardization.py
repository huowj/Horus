...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan 20 16:06:29 2018

@author: hwj
"""
##########I think standardization is not a good suggest###########
##########I think it is a not a good method#######################
...
#####################this method has no pass through####################
def DoubleWindowStandardization(value,m,n):
    T_length=len(value)
    T_dist=[0]*(T_length)
    for i in range(m,T_length-n+1):
        big_window=value[i-m:i+n]
        big_window_standardization=preprocessing.scale(big_window)
        nearest_neighbor_dist=np.infty
        for j in range(0,m-n+1):
            a=np.array(big_window_standardization[m:m+n])
            b=np.array(big_window_standardization[j:j+n])
...
if __name__ == "__main__":
    DoubleWindowStandardization(value,100,5)