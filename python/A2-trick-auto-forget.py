...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 13 15:00:53 2018
...
#data=pd.read_csv("/hwj/yahoo/python/data/A2Benchmark/synthetic_61.csv")
#value=data.value
...
if __name__ == "__main__":
    A2_p=pd.read_csv("/hwj/yahoo/distance/tricks/A2.csv")
    ...
    for i in range(100):
        data=pd.read_csv("/hwj/yahoo/python/data/A2Benchmark/synthetic_"+str(i+1)+".csv")
        value=data.value
        is_anomaly=data.is_anomaly
        a=DoubleWindow(value,norm_window[i],norm_sub[i])
        predict=norm(a,norm_transition[i],norm_a[i],0.9)
        ...
        table[i,2]=fp
        print(i)
    np.savetxt("/hwj/yahoo/distance/tricks/A2_result_all_0.9.csv", table, delimiter = ',')
